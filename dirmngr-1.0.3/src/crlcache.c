               ...
               int force_refresh)
{
  ...
  if (force_refresh)
    {
      dirmngr_isotime_t tmptime;
      
      if (*entry->last_refresh)
        {
          copy_time (tmptime, entry->last_refresh);
          add_isotime (tmptime, 30 * 60);
...
crl_cache_insert (ctrl_t ctrl, const char *url, ksba_reader_t reader)
{
  ...
  copy_time (entry->this_update, thisupdate); 
  copy_time (entry->next_update, nextupdate); 
  copy_time (entry->last_refresh, current_time);
